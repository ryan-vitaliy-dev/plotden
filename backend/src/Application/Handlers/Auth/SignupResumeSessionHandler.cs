...
{
    public class SignupResumeSessionHandler(
        ILogger<SignupResumeSessionHandler> logger, 
    ...
    {
        private readonly ILogger<SignupResumeSessionHandler> _logger = logger;
        ...
        public async Task<ServiceResult<CreatedSession>> HandleAsync(string rawToken, ClientInfo clientInfo, CancellationToken clt)
        {
            ...
            {
                return ServiceResult<CreatedSession>.Failure(findTokenResult.ErrorCode!.Value); // TODO: Check if this is ok
            }
            ...
            {
                _logger.LogError(
                    "Failed to find matching account with id {AccountId} for token hash {TokenHash} - Error: {ErrorCode}", 
                    token.AccountId, 
                    token.TokenHash,
